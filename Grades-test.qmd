...
The histogram shows that most students are very young, with ages concentrated between 20 and 23 years old, and more than 250 students at age 21–22. After age 25, the number of students drops, indicating that older students are rare in the dataset.
...
We can see that the mean in this course is around 7.
...
This histogram shows that most students have between 62 and 65 grades, with very few students below 60 or above 66. The distributions for males and females are very similar, indicating no meaningful difference between sexes in the total number of grades received.
...
```{r}
pass_summary <- avg_per_course |>
  summarise(
    min_course_avg = min(mean_grade),
    sem1_avg = mean(mean_grade[semester == 1]),
    sem2_avg = mean(mean_grade[semester == 2]),
    .by = id
  ) |>
  mutate(
...
pass_table <- pass_summary |>
  inner_join(final_df |> select(id, group, sex, final_grade), by = "id") |>
...
```{r}
fail_high <- pass_table |>
  filter(!pass, final_grade >= 10)

n_fail_high <- nrow(fail_high)
n_fail_high
```

105 students do not pass despite a final grade superior or equal to 10.
...
ggplot(pass_rate_group, aes(x = group, y = pass_rate)) +
  geom_col() +
  labs(x = "Group", y = "Pass rate")
```

The graph shows strong heterogeneity in academic success across groups. As we saw in the previous representations, a few groups (8, 11, 1) show relatively high pass rates, but most of the groups have low success rates. This suggests major disparities in performance.

### Question 29

```{r}
failing_ids <- pass_table |>
  filter(!pass) |>
  pull(id)

course_fail_counts <- avg_per_course |>
  filter(id %in% failing_ids, mean_grade < 5) |>
  summarise(
...
The course most responsible of students failure owing to grades below than 5 is Combat and Martial Arts of the Gods.
...
age_success <- pass_age |>
  summarise(
    success_rate = mean(pass),
    .by = age
  )

ggplot(age_success, aes(x = factor(age), y = success_rate)) +
  geom_col(fill = "steelblue") +
  labs(
    x = "Age",
    y = "Success rate",
...
Success rate is generally low across all ages, with no meaningful upward or downward trend. A few isolated ages (31 and 33) show artificially high success rates, likely due to very small sample sizes rather than a real age effect.
